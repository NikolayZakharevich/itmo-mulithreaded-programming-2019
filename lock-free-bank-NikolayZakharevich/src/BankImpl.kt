...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            if (account is AcquiredAccount && op !== account.op) {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        val from: AtomicRef<Account?> = atomic(null)
        val to: AtomicRef<Account?> = atomic(null)
        ...
        override fun invokeOperation() {
            ...
            if (fromIndex < toIndex) {
                from.compareAndSet(null, acquire(fromIndex, this))
                to.compareAndSet(null, acquire(toIndex, this))
            } else {
                to.compareAndSet(null, acquire(toIndex, this))
                from.compareAndSet(null, acquire(fromIndex, this))
            }
            ...
            when {
                amount > f.amount -> errorMessage = "Underflow"
                t.amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                else -> {
                    accounts[fromIndex].compareAndSet(f, AcquiredAccount(f.amount - amount, this))
                    accounts[toIndex].compareAndSet(t, AcquiredAccount(t.amount + amount, this))
                }
            }
